...
public class Koopa extends GameSprite implements Hitable {
	...
	public Koopa(String type, int x, int y) {
		super(type, x, y);
		vX = -0.5;
		vY = 0;
		animationSetup(6,1);
	...
	public void update() {
		...
		if (!dead && !Game.currentPlayer.deathAnimationPlaying && screenX + size > Game.currentPlayer.getX() && screenX < Game.currentPlayer.getX() + Game.currentPlayer.width && screenY + size > Game.currentPlayer.getY() && screenY < Game.currentPlayer.getY() + Game.currentPlayer.length) {
			...
			if (Game.currentPlayer.getY() + Game.currentPlayer.length <= screenY + 10) {
				if (Game.currentPlayer.mode == 9) {
					...
					LevelManager.spriteList.add(new Shell("s",(int)x,(int)y));
					dead = true;
				}
			} else {
				Player.die2();
	...
	public void draw(Graphics g) {
		...
		if (displayX+size+dX + scaleX/2 > ScaleManager.screenOffsetX && displayX + dX - scaleX/2 < Main.c.getWidth() - ScaleManager.screenOffsetX && displayY+size+dY + scaleY/2 > ScaleManager.screenOffsetY && displayY+dY - scaleY/2 < Main.c.getHeight() - ScaleManager.screenOffsetY) {
			if (vX < 0) {
			//p.drawImage(ResourcePath.getImage(t), displayX + dX - scaleX/2, displayY+dY - scaleY/2, displayX+size+dX + scaleX/2, displayY+size+dY + scaleY/2,basecoord[0],basecoord[1],basecoord[2],basecoord[3], null);
			...
			//p.drawImage(ResourcePath.getImage(t), displayX+size+dX + scaleX/2, displayY+dY - scaleY/2, displayX + dX - scaleX/2, displayY+size+dY + scaleY/2,basecoord[0],basecoord[1],basecoord[2],basecoord[3], null);
	...
	public void hitSide(boolean direction) {
		...
		int[] coord = SpriteSheetController.spriteSheetCoord(ResourcePath.getImage(t), 3, this.sheetrow, this.sheetcol);
		if (direction == false) {
			Game.gameComponentList.add(new EnemyDeath((int)displayX,(int)displayY,ResourcePath.getImage("k"),-0.5 * 2,-0.5, coord,size * 2,size * 2));
		} else {
			Game.gameComponentList.add(new EnemyDeath((int)displayX,(int)displayY,ResourcePath.getImage("k"),0.5 * 2,-0.5, coord,size * 2,size * 2));
		}
		//Game.gameComponentList.add(new BreakParticle(this.screenX + 15,this.screenY - 15,img,1d,-0.2d));
